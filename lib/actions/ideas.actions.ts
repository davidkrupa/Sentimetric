...
export const createIdeasFromProfile = async (): Promise<void> => {
  try {
    ...
    for (let i = 0; i < allAnalysis.length; i++) {
      analysisPrompt += `Topic of the analysis: "${allAnalysis[i].topic}", Content of the analysis: "${allAnalysis[i].content}" <br/>`;
    }
